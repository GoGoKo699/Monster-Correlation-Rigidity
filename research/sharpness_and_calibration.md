...
The core proves the following sufficient criterion in an exact simple unitary rational C2-cofinite holomorphic VOA of CFT type, central charge 24, and V1=0. For two real unit weight-two primaries x,y, put M=46/sqrt(141). If

$$f(x)\ge M-\epsilon_x,\qquad f(y)\ge M-\epsilon_y,\qquad
|\langle x,y\rangle+1/47|\le\delta,$$

with 0<=epsilon_x,epsilon_y<1/50 and
...
Let w1,w2 be real PCT-fixed vectors of exact ambient weight two. They need not be primary, mutually orthogonal, or normalized. The stress tensor omega is known and has squared norm twelve. Define seven real numbers

$$n_i=\langle w_i,w_i\rangle,\quad
\tau_i=\langle\omega,w_i\rangle,\quad
q_i=\langle w_i*w_i,w_i\rangle\quad (i=1,2),\qquad
m=\langle w_1,w_2\rangle.$$

Here *=the weight-two mode product a_(1)b. The q_i are these algebraic coefficients. For nonprimary w_i they must not be naively equated to a primary-only conformal position formula without including stress-tensor Ward terms.

Set

$$z_i=w_i-\frac{\tau_i}{12}\omega,\qquad N_i=n_i-\frac{\tau_i^2}{12}.$$

Because V1=0, a weight-two vector orthogonal to omega is primary: L1 maps it to V1=0, L2 is its pairing with omega times the vacuum, and higher positive modes vanish by degree. Thus z_i is primary, and its squared norm is N_i. Assume N_i>0. Put x_i=z_i/sqrt(N_i). Then

$$\boxed{ f(x_i)=
\frac{q_i-\tau_i n_i/2+\tau_i^3/36}{N_i^{3/2}},\qquad
\langle x_1,x_2\rangle=
\frac{m-\tau_1\tau_2/12}{\sqrt{N_1N_2}}. }\tag{1}$$

These exact identities remove both stress contamination and scale from the input fields.

**Derivation.** Write w=z+alpha omega, alpha=tau/12 and z perpendicular to omega. Since omega*z=2z, omega*omega=2omega and ||omega||^2=12, invariance of the form gives

$$q(w)=q(z)+6\alpha\|z\|^2+24\alpha^3,\qquad
n=\|z\|^2+12\alpha^2.$$

Solving for q(z) gives q-tau n/2+tau^3/36. The bilinear projection identity gives m-tau1 tau2/12. Division by the positive primary norms proves (1).
...
The three stress vectors e,f,g=omega-e-f have charges 1/2,1/2,23 and form a real weight-two subalgebra with coordinate-wise product 2xy and metric diag(1/4,1/4,23/2). This does not assert that the whole moonshine theory factorizes into three tensor factors.
...
Then a and b are exact maximizing primary directions, <a,b>=-r, and v is nonzero and perpendicular to a. Consider the exact unit-primary curve
...
$$\boxed{
\sqrt{141}\, f(x_t)=
\frac{46-3d t^2+k t^3}{(1+d t^2)^{3/2}},\qquad
...
$$\boxed{
\lim_{t\to0}
\frac{\operatorname{dist}(x_t,\mathcal A)}{\sqrt{M-f(x_t)}}
=\sqrt{\frac{\sqrt{141}}{72}}>0.
}\tag{4}$$

Here A is the full set of maximizing primary directions in the actual theory, not only the displayed pair.

If a value-to-nearest-axis bound dist(x,A)<=C(M-f(x))^alpha held for any finite C and alpha>1/2 throughout a neighborhood, (3) would imply |t|<=constant |t|^(2alpha), a contradiction as t tends to zero. The exponent 1/2 is thus optimal already in this fixed theory. This is not a claim that the coefficient 1/sqrt(2), the entry radius 1/10, or the numerical pair tolerances are optimal.
...
The prior core/checker/report and original license are copied byte-identically from the supplied research-completion packet; its 14 payload hashes and 63-check report were verified under normal/-O/-OO. No private planning or manuscript is included. Imported FQS, Ising-module, Sakuma and ALY theorems remain distinct source dependencies; no independent expert or formal verification of those results is claimed.
...
[LS] C.H. Lam, H. Shimakura, *Ising vectors in the vertex operator algebra V_Leech^+ associated with the Leech lattice*, arXiv:0810.5395v1, Theorem 3.1. Printed page 6 was visually inspected in this continuation. The lattice inclusion into moonshine and real-unitary convention are inherited from the source construction; this finite stress-algebra check does not reprove the construction.
https://arxiv.org/abs/0810.5395

[ALY] T. Abe, C.H. Lam, H. Yamada, *A remark on Z_p-orbifold constructions of the Moonshine vertex operator algebra*, arXiv:1705.09022v4, Theorem A.1. The exact endpoint is imported.

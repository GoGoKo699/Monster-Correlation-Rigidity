...
3. For the fixed group G=A5 x A5, there is a faithful **reducible** real orthogonal family of dimension D=16k+4 with fixed absolute class gap q=1/3, separation squared 24k/D, and class-mean image error squared 8/D, but exact normalizer distance squared **24k/D**. The approximation is even uniform on the entire group, with squared error 10/D. The distance stays bounded away from zero as the error tends to zero.
